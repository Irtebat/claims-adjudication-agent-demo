...
from predict import load_candidate, predict_fn
...
def run(args) -> dict:
    ...
    if not getattr(args, "candidate_version", None):
    ...
    if args.scorer_tier != "exact":
        with mlflow.start_run(
            run_name=f"eval-pilot-v{candidate_version}-{run_timestamp}",
            tags={**tags, "tier": "pilot"},
        ):
        ...
        cost = None
    with mlflow.start_run(
        run_name=f"eval-exact-v{candidate_version}-{run_timestamp}",
        tags={**tags, "tier": "exact"},
    ):
    ...
    if run_judges:
        ...
        with mlflow.start_run(
            run_name=f"eval-judges-v{candidate_version}-{run_timestamp}",
            tags={**tags, "tier": "judges"},
        ):
    ...
    if pilot:
        manifest["pilot_run_id"] = pilot.run_id
    if judge:
        manifest["judge_run_id"] = judge.run_id
    metrics = {
        "release_gate": _metric_summary(exact.metrics),
        "raw_exact_metrics": exact.metrics,
    }
    if judge:
        metrics["diagnostic_judges"] = {
            "authority_guidelines": {
                "value": judge.metrics.get("authority_guidelines/mean"),
                "status": "measured",
            },
            "retrieval_groundedness": {
                "value": judge.metrics.get("retrieval_groundedness/mean"),
